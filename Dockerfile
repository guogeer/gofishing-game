FROM hub.atomgit.com/amd64/golang:1.21.1-bookworm AS gobuilder
...
FROM hub.atomgit.com/amd64/node:20.6.1-bookworm AS nodebuilder
WORKDIR /build
...
COPY --from=nodebuilder /build/dist demo
